...
def generar_combinaciones_jugadores(jugadores,equiposAGenerar):
    """
    Este método utiliza la función combinations de la biblioteca itertools para generar 
    todas las combinaciones posibles de jugadores tomando equiposAGenerar jugadores a la vez. 
    Devuelve una lista de todas las combinaciones generadas.
    """
    return list(itertools.combinations(jugadores, equiposAGenerar))

def generar_equipos(combinaciones_jugadores):
    """
    Este método genera una lista de nombres de equipos utilizando una comprensión de lista. 
    Cada nombre de equipo se forma concatenando la cadena "Equipo" con un número 
    que va desde 1 hasta el número total de combinaciones de jugadores más 1
    """
    return [f'Equipo{i}' for i in range(1, len(combinaciones_jugadores) + 1)]

def calcular_peso_equipo(jugadores_equipo):
    """
    Este método calcula el peso total de un equipo sumando los valores de peso de cada jugador en el equipo. 
    Recibe como entrada una lista de diccionarios de jugadores 
    y devuelve un número entero que representa el peso total del equipo.
    """
    return sum(jugador['peso'] for jugador in jugadores_equipo)

def validar_enfrentamiento(jugadores_equipo1, jugadores_equipo2):
    """
    Este método verifica si hay algún jugador en común entre los dos equipos. 
    Recibe como entrada dos listas de diccionarios de jugadores que representan 
    los jugadores de cada equipo. Devuelve un valor booleano: 
    True si no hay jugadores en común y False si hay al menos un jugador en común.
    """
...
def obtener_enfrentamientos(enfrentamientos, combinaciones_jugadores, equipos):
    ...
    Verifica si el enfrentamiento es válido utilizando el método validar_enfrentamiento().
